...
final class CoreMIDIOutputService: MIDIOutputServiceProtocol {
    private let logger = Logger(subsystem: "com.chiimagnus.PianoKey", category: "CoreMIDIOutput")
    ...
    private func ensureClientAndPort() throws {
        if clientRef == 0 {
            let status = MIDIClientCreate("PianoKeyMIDIOutClient" as CFString, nil, nil, &clientRef)
            guard status == noErr else { throw MIDIOutputServiceError.clientCreate(status) }
        }

        if outputPortRef == 0 {
            let status = MIDIOutputPortCreate(clientRef, "PianoKeyMIDIOutPort" as CFString, &outputPortRef)
