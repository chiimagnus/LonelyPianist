...
| visionOS app | `LonelyPianistAVP/LonelyPianistAVPApp.swift` | 通过 `AppCompositionRoot` 组装依赖，注入 `AppRouter`，启动 WindowGroup + ImmersiveSpace |
...
| visionOS scheme | 本地手动运行 | `xcodebuild test -project LonelyPianist.xcodeproj -scheme LonelyPianistAVP -destination 'platform=visionOS Simulator,name=Apple Vision Pro' CODE_SIGNING_ALLOWED=NO` |
| Python service | 本地手动运行 | `python -m uvicorn server.api.main:app --host 0.0.0.0 --port 8765`（或 `./scripts/run_server.sh`） |

## 更新记录（Update Notes）
- 2026-05-06: 同步 Python 服务入口为 `server/api/main.py`（目录重组为 `api/`、`engines/`、`media/`）。
- 2026-05-10: 同步 AVP 主流程重构：引入 `AppRouter` + `FlowState` 的 root 切换，移除主流程 `NavigationStack` 依赖，曲库改为“内置条目 + 用户导入索引”合并展示（不再启动 seeding）。
...
| visionOS Practice | 定位、自动推进、按键贴皮高亮引导、虚拟钢琴模式 | [modules/lonelypianist-avp-practice.md](modules/lonelypianist-avp-practice.md) |
...
- 2026-04-30: 新增虚拟钢琴入口文件和功能地图条目。
- 2026-05-01: AVP 练习引导从光柱改为琴键贴皮高亮（decal），并移除 correct/wrong feedback 与 immersive pulse。
- 2026-05-02: 移除对 `.github/workflows/` 的假设（当前仓库不含 CI workflows），并同步虚拟钢琴放置引导改为 gaze-plane + 双手确认。
- 2026-05-05: 同步 AVP Bonjour 自动发现 + HTTP `/generate` 后端接入，以及 Python `/upload-expand` + `static/` 前端与一键启动脚本。
