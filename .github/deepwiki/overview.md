...
- **核心场景**：
  ...
  3. 谱面（PDF/图片）转 MusicXML 并在 AVP 中做分步引导。

## 一句话心智模型
- 仓库由三条运行面组成：**macOS 前端控制台**、**visionOS 空间引导端**、**本机 Python 智能后端**。
- macOS 负责实时 MIDI 与交互编排；Python 提供推理与 OMR；visionOS 消费 MusicXML + 手部追踪做空间引导。
...
| Python 服务 | `piano_dialogue_server/` | `WS /ws` 对话推理、`POST /omr/convert` 转谱 | `piano_dialogue_server/server/main.py` |
| OMR CLI/打包 | `piano_dialogue_server/omr/` | 将 PDF/图片转为 MusicXML，可打包分发 | `piano_dialogue_server/omr/cli.py` |
...
| `piano_dialogue_server/server/` | FastAPI + WebSocket 协议与推理接线 | Dialogue/OMR 服务契约与行为边界 |
| `piano_dialogue_server/omr/` | OMR 预处理与转换流水线 | PDF/图片到 MusicXML 的关键路径 |
...
| `score.musicxml` | OMR 流水线 (`oemer`) | AVP Import 或外部消费 | 每个 job 在 `out/omr/<job>/output/` |
...
| OMR 到 AVP | 选择谱面并转换 | preprocess -> oemer -> MusicXML -> AVP 导入 -> step 构建 | AR Guide 可进入引导态 |
...
- OMR 多页 PDF 默认只取第 1 页，非 page=1 会触发显式错误。
