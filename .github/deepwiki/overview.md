...
| AVP virtual piano | `LonelyPianistAVP/ViewModels/ARGuideViewModel.swift` | 虚拟钢琴开关、gaze-plane 放置引导、WorldAnchor 复用、键盘几何生成与渲染触发 |
...
| Python service | 本地手动运行 | `python -m uvicorn server.main:app --host 0.0.0.0 --port 8765`（或 `./scripts/run_server.sh`） |
