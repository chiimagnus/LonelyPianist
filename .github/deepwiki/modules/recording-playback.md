...
| 入口 / 类型 | 位置 | 何时触发 | 结果 |
| --- | --- | --- | --- |
| `startRecordingTake()` | `PianoKeyViewModel.swift` | 用户点 Rec | `recorderMode = .recording` |
| `stopTransport()`（recording 分支） | `PianoKeyViewModel.swift` | 点 Stop | 生成并保存 Take |
| `playSelectedTake()` | `PianoKeyViewModel.swift` | 点 Play | 启动回放与 playhead 时钟 |
| `seekPlayback(to:)` | `PianoKeyViewModel.swift` | 拖动 slider | 异步重启播放到新 offset |
...
- `PianoKeyViewModelRecorderStateTests` 验证播放不触发注入。
...
- `PianoKey/Views/Recording/PianoRollView.swift`
