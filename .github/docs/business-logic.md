...
PianoKey 是一个 macOS 菜单栏应用，把 MIDI 键盘输入转化为系统可见的文本、组合键或快捷指令触发。目标用户是希望用琴键替代部分键盘输入的创作者与效率工具用户。核心体验是“弹下音符即可触发预设动作”。主要输入是 MIDI `note on/off` 事件与用户配置规则；主要输出是前台应用中的实际输入效果、运行状态与事件日志。
...
3. 故障恢复流程：发现无输出 -> 查看 Status 与 Sources -> 刷新 MIDI 来源或补齐授权 -> 再次测试并确认事件计数增长。
...
flowchart TD
    ...
    E -- 否 --> F[显示连接状态与等待信息]
    F --> D
    E -- 是 --> G[映射引擎匹配 单键/和弦/旋律/力度]
    G --> H[执行动作: 文本/组合键/快捷指令]
    H --> I[更新预览与最近事件]
    I --> D
```
...
2. `PianoKey/ViewModels/PianoKeyViewModel.swift`：监听状态、权限流程、规则执行编排中心。
...
5. `PianoKey/Views/Main/ControlPanelView.swift`：控制面板核心业务界面入口。
